...
+ (void)initialize {
    LOG_LEVEL_ABBR[LOG_LEVEL_VERBOSE] = @"V";
    LOG_LEVEL_ABBR[LOG_LEVEL_DEBUG] = @"D";
    LOG_LEVEL_ABBR[LOG_LEVEL_INFO] = @"I";
    LOG_LEVEL_ABBR[LOG_LEVEL_PERFORMANCE] = @"P";
    LOG_LEVEL_ABBR[LOG_LEVEL_WARNING] = @"W";
    LOG_LEVEL_ABBR[LOG_LEVEL_ERROR] = @"E";
    
    
}

+ (U13LogLevel)level { return CURRENT_LOG_LEVEL; }
...
+ (NSString *)nameForLevel:(U13LogLevel)level {
    ...
    if (!LOG_LEVEL_NAMES[0]) {
        LOG_LEVEL_NAMES[LOG_LEVEL_VERBOSE] = NSLocalizedString( @"Verbose", @"Verbose" );
        LOG_LEVEL_NAMES[LOG_LEVEL_DEBUG] = NSLocalizedString( @"Debug", @"Debug" );
        LOG_LEVEL_NAMES[LOG_LEVEL_INFO] = NSLocalizedString( @"Info", @"Info" );
        LOG_LEVEL_NAMES[LOG_LEVEL_PERFORMANCE] = NSLocalizedString( @"Performance", @"Performance" );
        LOG_LEVEL_NAMES[LOG_LEVEL_WARNING] = NSLocalizedString( @"Warning", @"Warning" );
        LOG_LEVEL_NAMES[LOG_LEVEL_ERROR] = NSLocalizedString( @"Error", @"Error" );
    }
...
+ (void)e:(const char *)fn line:(int)line msg:(NSString *)msg {
	[self log:LOG_LEVEL_ERROR fn:fn line:line msg:msg];
}

+ (void)w:(const char *)fn line:(int)line msg:(NSString *)msg {
	[self log:LOG_LEVEL_WARNING fn:fn line:line msg:msg];
}

+ (void)i:(const char *)fn line:(int)line msg:(NSString *)msg {
	[self log:LOG_LEVEL_INFO fn:fn line:line msg:msg];
}
...
+ (void)t:(const char *)fn line:(int)line msg:(NSString *)msg start:(U13LOG_T_UNITS)start cutoff:(U13LOG_T_UNITS)cutoff {
    ...
    if (duration > cutoff)
        [self log:LOG_LEVEL_PERFORMANCE fn:fn line:line msg:[NSString stringWithFormat:@"%@: %llu", msg, duration]];
}

+ (void)d:(const char *)fn line:(int)line msg:(NSString *)msg {
	[self log:LOG_LEVEL_DEBUG fn:fn line:line msg:msg];
}

+ (void)v:(const char *)fn line:(int)line msg:(NSString *)msg {
	[self log:LOG_LEVEL_VERBOSE fn:fn line:line msg:msg];
}
